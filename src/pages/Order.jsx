...
import { backendMode, createRemoteOrder, getRemoteUser, saveRemoteProject, supabase, startPesapalPayment } from '../lib/backend';
...
export default function Order() {
  ...
  async function submit(e) {
    ...
    try {
      const id = makeId();
      const draft = { id, ...form, userEmail: user?.email || form.email, productId: product.id, productName: product.name, productType: product.type, price: product.price, paymentMethod, paymentStatus: paymentMethod === 'demo' ? 'Paid' : 'Pending', status: 'New', createdAt: new Date().toISOString(), requirements: form.notes };
      ...
      if (paymentMethod === 'demo') updateOrder(id, { paymentReference: 'DEMO-' + id.slice(-6), projectId }); else updateOrder(id, { projectId });
      setOrderId(id); setSubmitted(true);
    } catch (err) { setOrderId(''); setSubmitted(false); alert(err.message || 'Unable to place order.'); }
  }
  if (submitted) return <section className="container page-section narrow"><div className="success-card"><div className="success-icon"><CheckCircle2/></div><div className="eyebrow">Order {orderId}</div><h1>{paymentMethod === 'demo' ? 'Payment test complete.' : 'We’ve got it.'}</h1><p>{paymentMethod === 'demo' ? 'This is a local payment simulation. No money was charged.' : paymentMethod === 'pesapal' ? 'Your payment checkout was started. Your order will update after server-side verification.' : 'Your order is saved. The next payment step can be handled manually.'}</p>{paymentMethod !== 'demo' && paymentMethod !== 'pesapal' && <div className="payment-instructions"><strong>Next step</strong><span>Send payment using your agreed method, then share the reference with BOSS support. An admin can mark the order paid.</span></div>}<div className="hero-actions centered"><Link className="primary-button" to="/dashboard">Open dashboard</Link><Link className="secondary-button" to="/websites">Keep browsing</Link></div></div></section>;
