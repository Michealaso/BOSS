...
const replies = {
  'support-ai': {
    greeting: 'Hi! I’m the Support AI for this business. Ask me about hours, services, pricing, or how to contact the team.',
    rules: [
...
export default function DemoChatbot() {
  ...
  return <section className="container page-section bot-demo-page"><div className="bot-demo-intro"><div><div className="eyebrow"><Bot size={14}/> Interactive preview</div><h1>{bot.name}</h1><p>{bot.description}</p></div><Link className="primary-button" to={`/product/chatbot/${bot.id}`}>Get this bot <ArrowRight size={16}/></Link></div><div className="bot-window"><div className="bot-window-header"><div className="bot-avatar"><Bot size={19}/></div><div><strong>{bot.name}</strong><span><i className="status-dot"/> Online</span></div><small>Interactive preview</small></div><div className="bot-messages">{messages.map((m, i) => <div className={`chat-msg ${m.from}`} key={i}>{m.text}</div>)}</div><div className="quick-row">{quick.map(q => <button key={q} onClick={() => send(q)}>{q}</button>)}</div><div className="chat-compose"><input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Ask the assistant…"/><button onClick={() => send()}><Send size={17}/></button></div></div></section>;
}
