...
async function getAuthenticatedUser(token,supabaseUrl,anonKey){
...
export default async function handler(req,res){
 ...
 const user=await getAuthenticatedUser(accessToken,SUPABASE_URL,process.env.VITE_SUPABASE_ANON_KEY);
 if(!user)return res.status(401).json({error:'Your BOSS session is no longer valid. Sign in again and retry payment verification.'});
 ...
 try{
  ...
  if(ref.includes('-BR-')){table='build_requests';const x=await db.from(table).select('*').eq('payment_reference',ref).eq('user_id',user.id).maybeSingle();if(x.error)return res.status(500).json({error:`Could not load build request: ${x.error.message}`});record=x.data;}
  else{const x=await db.from(table).select('*').eq('payment_reference',ref).eq('user_id',user.id).maybeSingle();if(x.error)return res.status(500).json({error:`Could not load order: ${x.error.message}`});record=x.data;}
  if(!record)return res.status(404).json({error:'BOSS payment record not found for this account.',merchantReference:ref});
  ...
  const {error:updateError}=await db.from(table).update({...next,updated_at:new Date().toISOString(),pesapal_tracking_id:trackingId}).eq('id',record.id).eq('user_id',user.id);
