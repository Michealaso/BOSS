...
export default async function handler(req,res){
 ...
 try{
  ...
  const base=PESAPAL_ENV==='live'?LIVE_BASE:SANDBOX_BASE;const token=await getToken(base,PESAPAL_CONSUMER_KEY,PESAPAL_CONSUMER_SECRET);const q=trackingId?`?orderTrackingId=${encodeURIComponent(trackingId)}`:'';
  const r=await fetch(`${base}/api/Transactions/GetTransactionStatus${q}`,{headers:{Accept:'application/json','Content-Type':'application/json',Authorization:`Bearer ${token}`}});const d=await r.json().catch(()=>({}));if(!r.ok)return res.status(502).json({error:d?.message||'Pesapal status lookup failed.',details:d});
  ...
  const status=String(d.payment_status_description||d.payment_status||'').toUpperCase();const receivedAmount=Number(d.amount);const receivedCurrency=String(d.currency||'').toUpperCase();const expectedAmount=Number(record.price);const good=status==='COMPLETED'&&Number.isFinite(receivedAmount)&&receivedAmount>=expectedAmount&&receivedCurrency===BOSS_CURRENCY;
  ...
  const {error:updateError}=await db.from(table).update({...next,updated_at:new Date().toISOString(),pesapal_tracking_id:trackingId||record.pesapal_tracking_id}).eq('id',record.id);if(updateError)return res.status(500).json({error:`Payment status was read but BOSS could not update the record: ${updateError.message}`});
