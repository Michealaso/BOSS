...
async function getToken(base,key,secret){
  ...
  if(!r.ok || !d.token){
    const message=d?.message||d?.error?.message||'Pesapal authentication failed.';
...
export default async function handler(req,res){
  ...
  try{
    const {SUPABASE_SERVICE_ROLE_KEY,PESAPAL_ENV='sandbox',PESAPAL_CONSUMER_KEY,PESAPAL_CONSUMER_SECRET,PESAPAL_CALLBACK_URL,PESAPAL_IPN_ID}=process.env;
    const supabaseUrl=process.env.VITE_SUPABASE_URL||process.env.SUPABASE_URL;
    ...
    let user;
    try{ user=await getAuthenticatedUser(userToken); }
    ...
    let pesapalToken;
    try{
      pesapalToken=await getToken(base,PESAPAL_CONSUMER_KEY,PESAPAL_CONSUMER_SECRET);
    }catch(error){
      const payload={error:error?.message||'Pesapal authentication failed.',provider:'Pesapal',environment:PESAPAL_ENV==='live'?'live':'sandbox'};
      return res.status(error?.status===401?401:502).json(payload);
    }
    ...
    if(!PESAPAL_IPN_ID) return res.status(503).json({error:'PESAPAL_IPN_ID is missing. Register the BOSS IPN URL and add its ID to the server environment.'});
    ...
    const d=await r.json().catch(()=>({}));
    if(!r.ok||!d.redirect_url) return res.status(502).json({error:d?.message||d?.error?.message||'Pesapal checkout could not be created.',provider:'Pesapal',details:d});
