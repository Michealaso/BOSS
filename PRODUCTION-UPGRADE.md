...
3. Flutterwave hosted checkout endpoint and signed webhook handler.
...
- A Supabase project.
- A Flutterwave merchant account and API secrets.
...
2. Connect Supabase.
3. Test Flutterwave in test mode with a test order.
...
3. Configure Flutterwave payment plans in UGX if you want subscriptions, then paste the three plan IDs into server environment variables.
4. Configure the Flutterwave webhook URL as `/api/flutterwave-webhook` on the deployed domain and set the same secret hash in `FLW_SECRET_HASH`.
